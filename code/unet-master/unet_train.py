...
model_checkpoint = keras.callbacks.ModelCheckpoint('unet_model.hd5', monitor='loss',verbose=1, save_best_only=True)

# model.compile(optimizer=optimizer, loss='binary_crossentropy', metrics=['accuracy'])
model.compile(optimizer=optimizer, loss=create_weighted_binary_crossentropy(0.1, 2), metrics=['accuracy'])
...
model = unet()
model.load_weights("unet_model.hd5")
...
# if not os.path.exists('casia2_test'):
#     os.mkdir('casia2_test')
# saveResult("casia2_test",results)
